...
#include "filesys.h"
 ...
 int read(int _fd, char *buf,  int size){
	 long off;
	int block, block_off, i, j;
	struct inode *inode;
	char *temp_buf;

	inode = sys_ofile[user[user_id].u_ofile[_fd]].f_inode;
	if (!(sys_ofile[user[user_id].u_ofile[_fd]].f_flag & FREAD)){
		printf("\nthe file is not opened for read\n");
		return 0;
	}
	temp_buf = buf;
	off = sys_ofile[user[user_id].u_ofile[_fd]].f_off;
	...
	sys_ofile[user[user_id].u_ofile[_fd]].f_off += size;
	return size;
}


 int write(int _fd, char *buf,  int size){
	long off;
	int block, block_off, i, j;
	struct inode *inode;
	char *temp_buf;
	...
	inode = sys_ofile[user[user_id].u_ofile[_fd]].f_inode;
	if  (!(sys_ofile[user[user_id].u_ofile[_fd]].f_flag & FWRITE)){
