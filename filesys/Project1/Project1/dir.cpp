...
void _dir(){
	...
	printf("\n CURRENT DIRECTORY :%s\ti_ino->%d\t",dir.dir_name,cur_path_inode->i_ino);    //2.dir 输出文件信息   pjy
	printf("当前共有%d个文件/目录\n",dir.size);
...
void chdir(char *dirname){
	...
	for (i=0; i<dir.size; i++){
		if(dir.direct[i].d_ino == 0){
			//for(j=DIRNUM-1;j>=0&&dir.direct[j].d_ino == 0;j--);				//3.反序问题    pjy
			for (j = i + 1; j <= DIRNUM - 1 && dir.direct[i].d_ino == 0; j++);
